...
int s21_sprintf(char *str, const char *format, ...) {
  ...
  while (success && format_idx < format_len) {
    if (format[format_idx] == '%') {
      ...
      success = s21_read_format_args_and_write_modified_vararg_to_str(
...
bool s21_read_format_args_and_write_modified_vararg_to_str(
  ...
  p_format_args pfa = {0};
  bool format_read_successfully = s21_read_substr_of_format_line_needed_for_arg(
      cur_format_pos, &pfa, n_read_chars_from_format_str_ptr);
  if (format_read_successfully) {
    *n_written_chars_to_str_ptr =
        s21_append_str_with_vararg_modified_with_its_format_args_as_line(
    ...
    *n_written_chars_to_str_ptr =
        s21_append_str_with_wrong_conversion_specification(cur_str_pos, &pfa);
...
bool s21_read_substr_of_format_line_needed_for_arg(
    const char *format_substr, p_format_args *pfa_ptr,
  ...
  *n_read_chars_ptr = 0;
  ...
  while (success && format_substr[*n_read_chars_ptr] != '\0' &&
    ...
    s21_size_t step = 1;
    success = s21_read_arg_if_found(format_substr + *n_read_chars_ptr, pfa_ptr,
...
bool s21_read_arg_if_found(const char *cur_pos_on_format_substr,
  ...
  if (format_arg_type_being_processed_idx == 0 &&
      s21_is_flag_arg(*cur_pos_on_format_substr)) {
    *is_arg_read = true;
    s21_set_flag_arg(*cur_pos_on_format_substr, pfa_ptr);
  } else if (format_arg_type_being_processed_idx == 1 &&
             s21_is_digit_char(*cur_pos_on_format_substr)) {
    *is_arg_read = true;
    *step = s21_set_width_or_precision_arg_from_str(
        ...
        &pfa_ptr->specified_format_arg_types.width);
             ...
             s21_is_dot_char(*cur_pos_on_format_substr)) {
    *is_arg_read = true;
    const int DOT_LEN = 1;
    if (s21_is_digit_char(*(cur_pos_on_format_substr + DOT_LEN))) {
      *step += s21_set_width_or_precision_arg_from_str(
      ...
      pfa_ptr->specified_format_arg_types.precision = true;
             ...
             s21_is_length_arg(*cur_pos_on_format_substr)) {
    *is_arg_read = true;
    s21_set_length_arg(*cur_pos_on_format_substr, pfa_ptr);
  } else if (format_arg_type_being_processed_idx == 4 &&
             s21_is_specifier_arg(*cur_pos_on_format_substr)) {
    *is_arg_read = true;
    s21_set_specifier_arg(*cur_pos_on_format_substr, pfa_ptr);

    if (!pfa_ptr->specified_format_arg_types.precision)
      s21_set_default_precision_for_specifier(
...
bool s21_is_flag_arg(char ch) { return ch == '+' || ch == '-' || ch == ' '; }

void s21_set_flag_arg(char format_char, p_format_args *pfa_ptr) {
  if (format_char == '-')
...
bool s21_is_digit_char(char ch) { return ch >= '0' && ch <= '9'; }

s21_size_t s21_set_width_or_precision_arg_from_str(
    const char *str, unsigned *where_to_store_read_num,
    bool *is_format_arg_type_specified) {
  unsigned long long int num = 0;
  s21_size_t n_read_chars = s21_read_positive_num_from_line(&num, str);
...
s21_size_t s21_read_positive_num_from_line(unsigned long long int *read_num_ptr,
                                           const char *line) {
  if (!line) return 0;
...
s21_size_t s21_get_len_of_num_from_line(const char *line) {
  if (!line) return 0;

  const char *INT_CHARS = "0123456789";
  return s21_strspn(line, INT_CHARS);
}
...
unsigned long long int s21_pow_of_ten(unsigned power) {
...
bool s21_is_dot_char(char ch) { return ch == '.'; }

bool s21_is_length_arg(char ch) { return ch == 'h' || ch == 'l'; }

void s21_set_length_arg(char format_char, p_format_args *pfa_ptr) {
  if (format_char == 'h')
...
bool s21_is_specifier_arg(char ch) {
  return ch == 'c' || ch == 'd' || ch == 'f' || ch == 's' || ch == 'u' ||
         ch == '%';
}

void s21_set_specifier_arg(char format_char, p_format_args *pfa_ptr) {
  if (format_char == 'c')
...
void s21_set_default_precision_for_specifier(unsigned *precision,
...
s21_size_t s21_append_str_with_wrong_conversion_specification(
    char *str, const p_format_args *pfa_ptr) {
  s21_size_t n_written = 0;

  s21_write_char_with_null_terminator(str, '%');
  n_written++;
  if (pfa_ptr->specified_format_arg_types.flags)
    n_written += s21_append_flags_to_str(str + n_written, pfa_ptr);
  if (pfa_ptr->specified_format_arg_types.width)
    n_written += s21_write_ull_to_str(pfa_ptr->width, str + n_written);
  if (pfa_ptr->specified_format_arg_types.precision) {
    s21_write_char_with_null_terminator(str + n_written, '.');
    n_written++;
    n_written += s21_write_ull_to_str(pfa_ptr->precision, str + n_written);
  }
...
void s21_write_char_with_null_terminator(char *str, unsigned char ch) {
  if (!str) return;
...
s21_size_t s21_append_flags_to_str(char *str, const p_format_args *pfa_ptr) {
  if (!str) return 0;
...
s21_size_t s21_write_ull_to_str(unsigned long long int num, char *str) {
  if (!str) return 0;

  s21_size_t n = s21_get_digits_count(num);
  for (s21_size_t i = n, j = 0; i > 0; --i, ++j) {
    unsigned long long int ten_in_power_of_unit = s21_pow_of_ten(i - 1);
    int unit_num = num / ten_in_power_of_unit;
    s21_write_char_with_null_terminator(str + j, s21_digit_to_char(unit_num));
...
s21_size_t s21_get_digits_count(unsigned long long int num) {
  if (num == 0) return 1;
...
s21_size_t s21_append_str_with_vararg_modified_with_its_format_args_as_line(
    char *str, va_list *s21_sprintf_va_list, const p_format_args *pfa_ptr) {
  bool success = true;

  sprintf_vararg vararg = s21_retrieve_vararg_corresponding_to_specifier(
      s21_sprintf_va_list, pfa_ptr);
  s21_size_t n_written_chars_to_str =
      s21_set_modified_vararg_with_its_format_args_as_line(str, &vararg,
                                                           pfa_ptr);
...
sprintf_vararg s21_retrieve_vararg_corresponding_to_specifier(
  ...
  if (pfa_ptr->specifier.c) {
    ...
    if (!pfa_ptr->specified_format_arg_types.length)
      vararg.is_positive = s21_check_is_num_positive_and_set_its_module_value(
...
bool s21_check_is_num_positive_and_set_its_module_value(long long *place_to_set,
                                                        long long num) {
...
s21_size_t s21_set_modified_vararg_with_its_format_args_as_line(
  ...
  if (s21_is_signed_number(&pfa_ptr->specifier) &&
      !s21_is_nan_double(&pfa_ptr->specifier, vararg_ptr)) {
    if (vararg_ptr->is_positive) {
      if (pfa_ptr->specified_format_arg_types.flags) {
        s21_size_t n_written = s21_add_flag_char_to_str(str, &pfa_ptr->flags);
  ...
  s21_size_t n_written =
      s21_write_data_to_str(str + meaningfull_part_len, pfa_ptr, vararg_ptr);
  ...
  if (!pfa_ptr->specifier.percentage &&
    ...
    s21_format_str_width(str, pfa_ptr, meaningfull_part_len);
...
bool s21_is_nan_double(const format_specifiers *specifier,
                       const sprintf_vararg *vararg_ptr) {
  return specifier->f && isnan(vararg_ptr->value.d);
}

s21_size_t s21_add_flag_char_to_str(char *str, const format_flags *flags) {
  s21_size_t n_written = 0;
...
s21_size_t s21_write_data_to_str(char *str, const p_format_args *pfa_ptr,
                                 const sprintf_vararg *vararg_ptr) {
  if (!str) return 0;

  s21_size_t n_written = 0;

  if (s21_is_number(&pfa_ptr->specifier)) {
    n_written = s21_write_num_with_precision_to_str(str, pfa_ptr, vararg_ptr);
  } else if (pfa_ptr->specifier.c || pfa_ptr->specifier.s) {
    if (pfa_ptr->specifier.c)
      n_written = s21_write_char_or_wide_char_to_str(
          str, &pfa_ptr->length_modifier, vararg_ptr);
    else if (pfa_ptr->specifier.s)
      n_written =
          s21_write_string_or_wide_string_to_str(str, pfa_ptr, vararg_ptr);
...
s21_size_t s21_write_num_with_precision_to_str(
    char *str, const p_format_args *pfa_ptr, const sprintf_vararg *vararg_ptr) {
  s21_size_t n_written = 0;

  if (pfa_ptr->specifier.f)
    n_written = s21_convert_double_num_with_precision_to_str(
        vararg_ptr->value.d, pfa_ptr->precision, str);
  else if (!(pfa_ptr->precision == 0 && vararg_ptr->value.ull == 0))
    n_written = s21_pad_with_insignificant_zeroes_integer_num_and_write_to_str(
        str, vararg_ptr->value.ull, pfa_ptr->precision);

  return n_written;
}

s21_size_t s21_convert_double_num_with_precision_to_str(double num,
                                                        unsigned precision,
                                                        char *str) {
  if (isnan(num)) return s21_write_special_double_value_to_str(str, "nan");
  if (isinf(num)) return s21_write_special_double_value_to_str(str, "inf");
  ...
  s21_size_t n_int_part_digits =
      s21_write_ull_to_str((unsigned long long)num, write_position);
  write_position += n_int_part_digits;
  s21_size_t n_written = n_int_part_digits;

  if (precision > 0) {
    s21_write_char_with_null_terminator(write_position, '.');
    write_position++;
    n_written++;

    s21_size_t n_fract_part_len = s21_fract_part_to_str(num, write_position);

    if (n_fract_part_len < precision)
      s21_memset(write_position + n_fract_part_len, '0',
                 precision - n_fract_part_len);
    if (n_fract_part_len > precision)
      s21_round_double_given_on_str(str, write_position, precision);
    n_written += precision;
  }
...
s21_size_t s21_write_special_double_value_to_str(
    char *str, const char *special_value_as_str) {
  s21_memmove(str, special_value_as_str, s21_strlen(special_value_as_str));
  return s21_strlen(str);
}

s21_size_t s21_fract_part_to_str(double num, char *str) {
  if (!str) return 0;
...
void s21_round_double_given_on_str(char *double_as_str, char *fract_part_ptr,
  ...
  if (s21_is_dot_char(*right_to_left_ptr)) {
    has_dot_right_before_rounding_point = true;
    right_to_left_ptr--;
  }
  int dot_and_carry_one = (s21_char_to_digit(*rounding_point_ptr) + 5) / 10;
  while (dot_and_carry_one == 1 && right_to_left_ptr != double_as_str) {
    int sum = s21_char_to_digit(*right_to_left_ptr) + dot_and_carry_one;
    *right_to_left_ptr = s21_digit_to_char(sum % 10);
    dot_and_carry_one = sum / 10;
    right_to_left_ptr--;
    if (s21_is_dot_char(*right_to_left_ptr)) right_to_left_ptr--;
  }
  if (right_to_left_ptr == double_as_str) {
    *right_to_left_ptr = s21_digit_to_char(dot_and_carry_one);
  }
...
int s21_char_to_digit(char ch) { return ch - '0'; }

char s21_digit_to_char(int digit) { return digit + '0'; }

s21_size_t s21_pad_with_insignificant_zeroes_integer_num_and_write_to_str(
    char *str, unsigned long long num, unsigned precision) {
  if (!str) return 0;

  s21_size_t n_ins_zeroes_written =
      s21_write_insignificant_zeroes_of_integer_num(str, num, precision);
  s21_size_t n_digits = s21_write_ull_to_str(num, str + n_ins_zeroes_written);

  return n_digits + n_ins_zeroes_written;
}

s21_size_t s21_write_insignificant_zeroes_of_integer_num(char *str,
                                                         unsigned long long num,
                                                         unsigned precision) {
  if (!str) return 0;

  s21_size_t n_digits = s21_get_digits_count(num);
...
s21_size_t s21_write_char_or_wide_char_to_str(
  ...
  if (!length_modifier->l) {
    s21_write_char_with_null_terminator(str,
                                        (unsigned char)vararg_ptr->value.ull);
    ++n_written;
  } else {
    s21_size_t n_chars_written =
        s21_write_to_str_converted_chars_from_wide_char(
...
s21_size_t s21_write_to_str_converted_chars_from_wide_char(char *str,
                                                           wchar_t wchar) {
  if (!str) return 0;
...
s21_size_t s21_write_string_or_wide_string_to_str(
    char *str, const p_format_args *pfa_ptr, const sprintf_vararg *vararg_ptr) {
  if (!str) return 0;
  ...
  if (!pfa_ptr->length_modifier.l) {
    n_written += s21_copy_n_chars_or_up_to_null_terminator(
        str, (const char *)vararg_ptr->value.cvptr, pfa_ptr->precision);
  } else {
    s21_size_t n_bytes_written =
        s21_write_wide_char_string_parsed_to_basic_string_to_str(
...
s21_size_t s21_copy_n_chars_or_up_to_null_terminator(char *dest,
                                                     const char *src,
                                                     s21_size_t count) {
  if (!src) return 0;
...
s21_size_t s21_write_wide_char_string_parsed_to_basic_string_to_str(
  ...
  while (success && wide_char_str[i] != L'\0' && can_write_entire_wc) {
    ...
    if (n != (s21_size_t)-1) {
      ...
      for (s21_size_t j = 0; j < n && can_write_entire_wc;
        ...
        s21_write_char_with_null_terminator(str++, buf[j]);
        n_written += 1;
      }
    } else {
      success = false;
...
void s21_format_str_width(char *str, const p_format_args *pfa_ptr,
